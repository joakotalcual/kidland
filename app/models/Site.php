...
class Site extends CI_Model
{
    ...
    public function getSaleJuegoByID($id = NULL) {
        if (!$id) {
            return FALSE;
        }

        // Selecciona las columnas requeridas
        $this->db->select('sales_juegos.*, juegos.nombre AS nombre_juego');

        // Realiza el JOIN con la tabla juegos
        $this->db->from('sales_juegos');
        $this->db->join('juegos', 'sales_juegos.juego_id = juegos.id', 'left');

        // Agrega la condición para el ID de venta de juego
        $this->db->where('sales_juegos.id', $id);
    ...
    public function getSaleJuegoDatosByID($id = NULL) {
        if (!$id) {
            return FALSE;
        }
        $this->db->where('sale_juego_id', $id);
        $q = $this->db->get('sales_juegos_personas');
        if ($q->num_rows() > 0) {
            return $q->result();
        }
        return FALSE;
    ...
    public function getTipoByID($id = NULL) {
        if ( ! $id) {
            return FALSE;
        }
        $q = $this->db->get_where('tipos', array('id' => $id), 1);
        if ( $q->num_rows() > 0 ) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getJuegoByID($id = NULL) {
        if ( ! $id) {
            return FALSE;
        }
        $q = $this->db->get_where('juegos', array('id' => $id));
        if ( $q->num_rows() > 0 ) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getTipoByJuegoID($id = null) {
        if (!$id) {
            return false;
        }

        // Realizar la consulta con JOIN para obtener el tipo del juego
        $this->db->select('tipos.nombre AS tipo_nombre');
        $this->db->from('juegos');
        $this->db->join('tipos', 'juegos.tipo = tipos.id');
        $this->db->where('juegos.id', $id);
        $this->db->limit(1);
        $query = $this->db->get();

        // Verificar si se encontraron resultados
        if ($query->num_rows() > 0) {
            return $query->row()->tipo_nombre; // Devolver el nombre del tipo
        } else {
            return false; // Si no se encuentra el juego, devolver FALSE
        }
    }

    public function getInsumoByID($id = NULL) {
        if ( ! $id) {
            return FALSE;
        }
        $q = $this->db->get_where('insumos', array('id' => $id));
        if ( $q->num_rows() > 0 ) {
            return $q->row();
        }
        return FALSE;
